          ...
          {companies.map((company, index) => (
            ...
            >
              ...
              <span className="company-tile-state">
                {company.status === "incoming" ? "incoming" : company.story ? "read the story ↗" : "story soon"}
              </span>
            </button>
          ))}
        </div>
        ...
        >
          <div className="company-story-content" key={activeCompany.name}>
            <div className="company-story-meta">
              <span>PORTCO {String(activeIndex + 1).padStart(2, "0")}</span>
              <span>{activeCompany.status ?? "portfolio note"}</span>
            </div>
            <p className="company-story-kicker">THE STORY BEHIND THE YES</p>
          ...
          <p className="company-story-instruction">hover / focus / tap another tile</p>
        </aside>
      </div>
    </section>
  );
